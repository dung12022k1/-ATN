...
<template>
  <a-form :form="form" :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }" >
    <div style="font-size: 30px;font-style: italic">Flight Edit</div>
    ...
    <a-form-item label="Departure At" >
      ...
      <span id="error-departureat" style=" color: red"></span>
    </a-form-item>
    <a-form-item label="Return At" >
      <a-date-picker v-model="form.ReturnAt"/>
      <span id="error-returnat" style=" color: red"></span>
    </a-form-item>
    <a-form-item label="Price" >
      <a-input v-model="form.Price"/>
      <span id="error-price" style=" color: red"></span>
    </a-form-item>
    <a-form-item label="Is Round Ticket" >
      <input type="checkbox" id="checkbox" v-model="form.IsRoundTicket" />
      <label for="checkbox">{{ checked }}</label>
<!--      <a-input v-model="form.IsRoundTicket"/>-->
      <span id="error-ticket" style=" color: red"></span>
    </a-form-item>
...
export default {
  ...
  created() {
    this.getlocation(),
    this.getFlights(this.$route.params.id)
  ...
  methods: {
    ...
    validate(){
      ...
      var errordepartureat = document.getElementById('error-departureat')
      var errorreturnat = document.getElementById('error-returnat')
      ...
      if (this.form.Price === undefined || this.form.Price === "" ) {
        ...
        if (this.form.DepartureAt === undefined || this.form.DepartureAt === "") {
          errordepartureat.innerText = 'Please choose flight Departure At'
          checkNumber++;
        } else {
          errordepartureat.innerText = ''
        }
        if (this.form.ReturnAt === undefined || this.form.ReturnAt === "") {
          errorreturnat.innerText = 'Please choose flight Return At'
          checkNumber++;
        } else {
    ...
    async handleSubmit(e){
      ...
      await FlightService.edit(this.form.Id,this.form).then(
